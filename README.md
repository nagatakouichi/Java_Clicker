...
①マルチスレッド処理の実装  
ポイントを自動的に増加させる処理のため、マルチスレッド処理を実装しました。ポイントの管理と装置の購入の処理を別スレッドで行うようにしています。終了処理の実装にてこずりましたが、UIの終了ボタンか×ボタンでウインドウを閉じてすべてのスレッドを終了させるような実装を行いました。  

②セーブ機能の実装  
シリアライズを利用して総ポイントや装置の購入数などのデータをセーブできるようにしました。また、ゲーム終了時に自動でセーブする機能を実装しました。  
